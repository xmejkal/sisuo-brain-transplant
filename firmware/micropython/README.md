...
stateDiagram-v2
    ...
    opening --> open: safety_cap_tripped
    ...
    closing --> idle: stroke_finished
    obstructed --> fault: retry_limit_reached
    obstructed --> open: safety_cap_tripped
    obstructed --> fault: sensor_failed
    obstructed --> open: stroke_finished
    fault --> idle: open_pressed
    fault --> idle: reset_requested
```
Generated by `python3 tools/fsm_diagram.py`, which reads the table in states.py. Regenerate it when the behaviour changes — this copy had drifted before anyone noticed, so `make check` now compares it.
